...
| 檔案 | 做什麼 | 掛在哪 |
|---|---|---|
| `coined_word_guard.py` | 掃詞：自創量詞、口語動詞、比喻、把機器狀態講成人的動作 | PreToolUse（改檔案、Bash、交辦、Artifact）與 Stop |
| `copy_judge.py` | 讀「他問的那一句」加上回覆，給 0 到 10 分 | PreToolUse（改檔案）與 Stop |
| `copy_gate_on_edit.py` | 這次編輯動到引號裡的中文，就要求先跑文案 skill | PreToolUse（改檔案） |
| `copy-voice-inject.py` | 每一輪把他自己寫過的句子送進 context | UserPromptSubmit |
| `complaint_learn.py` | 他一說「聽不懂」就分析上一輪問答，分四種原因 | UserPromptSubmit |

## copy-rules/　規則與情境

| 檔案 | 放什麼 |
|---|---|
| `情境目錄.json` | 12 種情境，各自要掌握什麼、不准寫什麼、送出前檢查什麼，每一條都附出處 |
...
| `judge-rubric.txt` | 判官的評分規範，externalized 出來才能改版重考 |
| `judge-score.json` | 最近一次考試的成績。`pass` 這個欄位決定判官准不准擋人 |
...
`gold.jsonl`、`eval-*`、`rejected.jsonl`、`complaints.jsonl`。
